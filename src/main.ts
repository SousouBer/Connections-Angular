...
import { groupMessagesReducers } from './app/store/reducers/group-messages.reducers';
import { GroupMessagesEffects } from './app/store/effects/group-messages.effects';
...
bootstrapApplication(AppComponent, {
  providers: [
    ...
    provideStore({profileData: profileReducers, groupsData: groupReducers, participantsData: participantsReducers, groupMessagesData: groupMessagesReducers}),
    provideStoreDevtools({
      maxAge: 25, // Retains last 25 states
      logOnly: !isDevMode(), // Restrict extension to log-only mode
      autoPause: true, // Pauses recording actions and state changes when the extension window is not open
    }),
    provideEffects([ProfileEffects, GroupsEffects, ParticipantsEffects, GroupMessagesEffects]),
  ],
});
